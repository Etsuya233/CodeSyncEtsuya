...
int main() {
	...
	if (dat) {
		...
		printf("! Error: Cannot find the database \"data.txt\" \n");
		printf("! Program Abort\n");
		printf("! Please create \"data.txt\" in the current directory\n");
...
int* find_core(int mode, char* id, char* name) {
	//Mode 1 to 2 represent the mode in the argument sheet by order
	int res[100] = { 0 }, am = 0;
	if (mode == 1) {
		for (int i = 0; i < AMOUNT; i++) {
